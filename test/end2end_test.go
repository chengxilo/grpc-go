...
<html><body>Hello World</body></html>`,
		...
		{
			name: "non-gRPC content-type with bytes payload length more than transport.NonGRPCBodyMaxLen",
			responses: []httpServerResponse{
				{
					headers: [][]string{
						{
					...
					payload: bytes.Repeat([]byte("a"), transport.NonGRPCBodyMaxLen+1),
				},
			},
			wantCode: codes.Unknown,
			wantErr: `rpc error: code = Unknown desc = unexpected HTTP status code received from server: 200 (OK); transport: received unexpected content-type "text/html"
DATA-----------------------------
` + strings.Repeat("a", transport.NonGRPCBodyMaxLen),
		},
		{
